# Kramak â€” Master Rules Inventory (v1.1 Transformation)
...
### 1. Bounded Autonomy & Strategic Mindset (`.kramak/planner/CORE.md` lines 1â€“40)
...
### 2. Grounded Planning & Capability Gate (`.kramak/planner/CORE.md` lines 43â€“72)

| # | Rule / Constraint | Destination |
|---|---|---|
| 12 | On invocation with `state.phase: "planning"`, derive project requirements from workspace files alone. | `PLANNER-CORE` |
| 13 | **Grounded Verification:** Every specification MUST be grounded in actual code read via toolsâ€”never write from memory or assumption. | `ROUTER`, `PLANNER-CORE` |
...
| 16 | **Gate Decision Routing:** Score $\ge 0.80$ grant full planning autonomy; $0.60 \le S < 0.80$ proceed with conservative scope (smaller batches, more ðŸ”´ Guided WIs); $< 0.60$ fail closed to `WAITING` with capability advisory. | `PLANNER-CORE`, `PLANNER-MODULE:capability-gate` |
...
### 3. Empty Workspace Guard (`.kramak/planner/CORE.md` lines 76â€“82)
...
### 4. Project Discovery & Structure Mapping (`.kramak/planner/CORE.md` lines 83â€“121)
...
### 5. Context Reading & Anti-Anchoring Bias (`.kramak/planner/CORE.md` lines 123â€“151)
...
### 6. INBOX Processing (`.kramak/planner/CORE.md` lines 152â€“163)
...
### 8. Strategic Vision (5 Lenses) (`.kramak/planner/CORE.md` lines 228â€“314)
...
| 51 | **Lens 1 (Quality Retrospective):** Read actual code of last 2â€“3 batches; assess whether it is genuinely excellent vs merely functional, thoughtful UX, and graceful errors. Output quality WIs or confirm quality. | `PLANNER-CORE` |
...
| 53 | **Lens 3 (Competitive & Market Scan):** Web search top 3â€“5 competitors; build feature comparison matrix; identify missing capabilities. Output new feature proposals. | `PLANNER-CORE` |
...
### 9. Perspective Selection (PERCEIVE $\rightarrow$ REASON $\rightarrow$ DECIDE) (`.kramak/planner/CORE.md` lines 315â€“422)
...
### 10. Prioritization by productPhase & Modification Boundaries (`.kramak/planner/CORE.md` lines 423â€“537)
...
### 11. Batch Planning & Sizing (`.kramak/planner/CORE.md` lines 538â€“614)
...
| 75 | **Task Sizing Horizon:** Each WI must represent $\le 2$ hours human-equivalent work (~200 word spec intent, informed by METR 80% reliability horizon). Typical batch size is 3â€“8 WIs. | `PLANNER-CORE` |
...
| 78 | **Story Build Order:** Build in dependency order: 1. Schema/data model (ðŸ”´ Guided) $\rightarrow$ 2. Backend endpoint/logic (ðŸŸ¡ Directed) $\rightarrow$ 3. Frontend component (ðŸŸ¡ Directed / ðŸŸ¢ Outcome) $\rightarrow$ 4. Integration wiring (ðŸŸ¡ Directed) $\rightarrow$ 5. Polish (ðŸŸ¢ Outcome). | `PLANNER-CORE` |
| 79 | **One Concern Per WI:** Group related changes coherently (3â€“8 files allowed if coherent under one concern). | `PLANNER-CORE` |

### 12. Branch Management (`.kramak/planner/CORE.md` lines 616â€“628)
...
### 13. Work Item Specification & Detail Scaling (`.kramak/planner/CORE.md` lines 630â€“800)
...
| 89 | **ðŸ”´ Guided Scope:** Auth, security, database schema migrations, data models, encryption, and cascading failure vectors. | `PLANNER-CORE`, `PLANNER-MODULE:output-contract` |
...
| 91 | Guided New Files: Mark `**Verified:** âœ… New file (no prior lines)` and `// BEFORE: (empty / new file)`. | `PLANNER-MODULE:output-contract` |
...
| 93 | **ðŸŸ¡ Directed Scope:** New API endpoints, refactoring existing features, component rewrites, integration wiring. | `PLANNER-CORE`, `PLANNER-MODULE:output-contract` |
...
| 96 | **ðŸŸ¢ Outcome Scope:** Creating new files, docs, configs, UI components from scratch, tests, DX improvements. | `PLANNER-CORE`, `PLANNER-MODULE:output-contract` |
...
### 14. Self-Audit Checklist (`.kramak/planner/CORE.md` lines 802â€“828)
...
| 101 | Check 3: Risk distribution is balanced ($\le 50\%$ ðŸ”´ Guided). | `PLANNER-CORE` |
| 102 | Check 4: Full Grounded Verification confirmed for all ðŸ”´ Guided items (grep, verbatim quote, unique match). | `PLANNER-CORE` |
| 103 | Check 5: Target files read and grounded type signatures provided for all ðŸŸ¡ Directed items. | `PLANNER-CORE` |
| 104 | Check 6: Clear, observable acceptance criteria for all ðŸŸ¢ Outcome items. | `PLANNER-CORE` |
...
### 15. Session Continuity & Model Handoff (`.kramak/planner/CORE.md` lines 830â€“903)
...
| 113 | Context Fatigue Warning: Account for LLM degradation at ~40â€“50% context capacity and the Lost in the Middle effect; prefer fresh sessions when in doubt. | `PLANNER-CORE` |
...
### 16. Executor Audit Review & Circuit Breaker (`.kramak/planner/CORE.md` lines 905â€“943)
...
### 17. Edge Case Handling Table (`.kramak/planner/CORE.md` lines 945â€“966)
...
| 126 | Feature requires data model change $\rightarrow$ Write WIs in dependency order; schema = ðŸ”´ Guided. | `PLANNER-CORE`, `PLANNER-MODULE:edge-cases` |
...
| 135 | Source code "quick fix" temptation $\rightarrow$ **RESIST.** Write a ðŸŸ¢ Outcome WI; tokens are for planning. | `PLANNER-CORE`, `PLANNER-MODULE:edge-cases` |
...
### 18. Development Principles (`.kramak/ROUTER.md` lines 1â€“312)
