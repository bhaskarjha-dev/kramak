...
1. Read `.agents/pipeline/state.json`
   - If it doesn't exist: read [path to kramak]/spec/BOOTSTRAP.md and bootstrap
   - If it exists: follow the procedure for state.phase

2. Phases:
   - planning → Follow [path to kramak]/spec/PLANNER.md
   - executing → Follow [path to kramak]/spec/EXECUTOR.md
   - auditing → Follow [path to kramak]/spec/PLANNER.md §STEP 7

3. Always read [path to kramak]/spec/PRINCIPLES.md first
