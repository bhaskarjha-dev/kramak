## Description

Brief description of what this PR does.

## Type of change

- [ ] Bug fix (fixes an issue with the methodology)
- [ ] New feature (adds new capability to the spec)
- [ ] Documentation (docs, adapters, examples)
- [ ] Adapter (new or updated IDE adapter)

## Anti-Bias Guard (required for `spec/` changes)

> If this PR modifies files in `spec/`, you must answer all 5 questions:

1. **WHAT failure mode does this change prevent?**
   →

2. **IS this useful for ALL types of work?**
   →

3. **SCENARIO TEST — Would this help in all 3?**
   - [ ] A backend data model migration batch
   - [ ] A frontend component feature batch
   - [ ] A security hardening batch

4. **COULD this change HURT a different type of work?**
   →

5. **COOLDOWN — Have you slept on it?**
   →

## Checklist

- [ ] I've read [CONTRIBUTING.md](../CONTRIBUTING.md)
- [ ] My changes follow the existing spec style
- [ ] I've tested this with at least one AI agent (if applicable)
